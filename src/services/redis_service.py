...
class RedisClient:
    ...
    async def get_payment_summary(self, from_date=None, to_date=None):
        default_summary = await self._get_processor_summary("default", from_date, to_date)
        fallback_summary = await self._get_processor_summary("fallback", from_date, to_date)
    ...
    async def _get_processor_summary(self, processor: str, from_date=None, to_date=None):
